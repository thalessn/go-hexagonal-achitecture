...
import (
	...
	"github.com/thalessn/go-hexagonal/adapters/cli"
	mock_application "github.com/thalessn/go-hexagonal/application/mocks"
)
...
func TestRun(t *testing.T) {
	...
	resultExpected = fmt.Sprintf("Prdocut %s has been enable.", productName)
	result, err = cli.Run(service, "enable", productId, productName, 0)
	require.Nil(t, err)
	require.Equal(t, resultExpected, result)

	resultExpected = fmt.Sprintf("Prdocut %s has been disabled.", productName)
